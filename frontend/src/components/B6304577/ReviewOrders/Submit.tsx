...
function Submit({ formDataRating, setFormDataRating, activeStep, setActiveStep, steps, checkedPaymentsAll }: any) {
    ...
    const [dateP, setDateP] = useState(1);
    const [year, setYear] = useState(2020);
    // console.log(year,month,dateP);
    

    //Test
    const onYearPicked = (res: any) => {
        setYear(res.selectedYear);
    };

    const onMonthPicked = (res: any) => {
        setMonth(res.selectedMonth);
    };

    const onDatePicked = (res: any) => {
        console.log(res);
        
        setDateP(res.selectedDay);
    };

    const onResetDate = (res: any) => {
        setMonth(res.selectedMonth);
        setDateP(res.selectedDay);
        setYear(res.selectedYear);
    };

    const onResetDefaultDate = (res: any) => {
        setMonth(res.selectedMonth);
        setDateP(res.selectedDay);
        setYear(res.selectedYear);
    };
